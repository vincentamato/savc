...
def process_midi(args):
    ...
    try:
        ...
        output_filename = Path(midi_path).name
        new_midi_path = output_dir / output_filename
...
def main():
    ...
    input_dir = Path('data/unique_midis')
    output_dir = Path('data/midis')
    ...
    midi_files = [(str(f), output_dir) for f in input_dir.rglob("*.mid*")]
    total_files = len(midi_files)
    
    logger.info(f"Found {total_files} MIDI files in {input_dir}")
